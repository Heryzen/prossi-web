...
async function fetchParentBySlug(slug: string): Promise<CmsTreatment | null> {
  const results = await directusFetch<CmsTreatment[]>(
    `/items/treatments?filter[category][_eq]=skin&filter[slug][_eq]=${encodeURIComponent(slug)}&filter[status][_eq]=published&limit=1&fields=id,name,slug,description,image`
  );
...
export default async function SkinTreatmentDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <section className="w-full pt-[140px] pb-[80px] px-6 md:px-[100px]" style={{ background: ACCENT }}>
      <div className="max-w-[1240px] mx-auto flex flex-col items-center gap-10">
        <nav className="w-full flex items-center gap-2 flex-wrap">
          <Link href="/treatments/skin-treatment" className="font-['Lato',sans-serif] text-[14px] text-white/80 hover:underline">
            Skin Treatment
          </Link>
