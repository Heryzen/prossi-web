  ...
  if (notFound || !order) {
    return (
      <div className="min-h-screen bg-[#f9f7f4] flex flex-col items-center justify-center gap-4">
        <p className="font-['Lato',sans-serif] text-[#3b4963]">Order tidak ditemukan.</p>
  ...
  return (
    <div className="min-h-screen bg-[#f9f7f4] pt-[104px]">
      <div className="sticky top-[104px] z-40 bg-white border-b border-[#e6ecf7] px-6 py-3">
        <Link href="/admin/orders" className="font-['Lato',sans-serif] text-[13px] font-semibold text-[#3b4963] hover:text-[#b59637]">
          ← Daftar Pesanan
        </Link>
      </div>
      ...
      <div className="px-4 md:px-8 py-6 max-w-[780px] mx-auto flex flex-col gap-4">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <p className="font-['Lato',sans-serif] text-[11px] font-semibold tracking-widest uppercase text-[#b59637] mb-0.5">Pesanan</p>
            <h1 className="font-['Lato',sans-serif] font-extrabold text-[26px] text-[#11151c]">
              #{order.order_number}
            </h1>
            <p className="font-['Lato',sans-serif] text-[13px] text-[#889bbf] mt-1">
          ...
          <span className={`mt-1 px-4 py-1.5 rounded-full font-['Lato',sans-serif] text-[12px] font-bold whitespace-nowrap ${
        ...
        {st === "pending_payment" ? (
          <div className="bg-[#fdf6ec] border border-[#f0d89a] rounded-[14px] px-5 py-4 flex items-center gap-3">
            ...
            <div>
              <p className="font-['Lato',sans-serif] font-semibold text-[14px] text-[#7a5c0a]">Menunggu Pembayaran</p>
              <p className="font-['Lato',sans-serif] text-[12px] text-[#a07820] mt-0.5">Belum ada konfirmasi pembayaran dari Midtrans.</p>
            </div>
          </div>
        ) : st !== "cancelled" ? (
                ...
                return (
                  <div key={step.key} className="flex items-center flex-shrink-0">
                    <div className="flex flex-col items-center gap-1.5">
                      ...
                      <span className={`font-['Lato',sans-serif] text-[10px] font-semibold whitespace-nowrap ${
          ...
          <div className="bg-[#fdf0ee] border border-[#f5cbc7] rounded-[14px] px-5 py-4 flex items-center gap-3">
            ...
            <div>
              <p className="font-['Lato',sans-serif] font-semibold text-[14px] text-[#a8312a]">Pesanan Dibatalkan</p>
              <p className="font-['Lato',sans-serif] text-[12px] text-[#c06060] mt-0.5">Order ini telah dibatalkan dan tidak bisa diproses kembali.</p>
            </div>
          </div>
        )}
        ...
        {!isFinal && !isPendingPayment && (
          <div className="bg-white rounded-[14px] border border-[#e6ecf7] px-5 py-5">
            <p className="font-['Lato',sans-serif] text-[11px] font-bold uppercase tracking-wider text-[#889bbf] mb-4">Aksi</p>
            {canShip ? (
              <div className="flex flex-col gap-3">
                <p className="font-['Lato',sans-serif] text-[14px] text-[#3b4963]">Paket siap dijemput kurir. Proses pengiriman sekarang.</p>
                <div className="flex gap-3 flex-wrap">
                  <button
                    onClick={() => { setShipError(""); setShowModal(true); }}
                    className="flex items-center gap-2 bg-[#b59637] rounded-[100px] px-7 py-3 text-white font-['Lato',sans-serif] font-semibold text-[14px] hover:opacity-90 transition-opacity"
                  >
                  ...
                  <button
                    ...
                    className="bg-white text-[#a8312a] border border-[#f5cbc7] hover:bg-[#fdf0ee] rounded-[100px] px-5 py-3 font-['Lato',sans-serif] font-semibold text-[14px] cursor-pointer transition-colors disabled:opacity-50"
                  >
                    Batalkan Pesanan
                  </button>
                </div>
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-[#f9f7f4] rounded-[10px] px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    <p className="font-['Lato',sans-serif] text-[13px] text-[#889bbf] mb-0.5">Langkah berikutnya</p>
                    <p className="font-['Lato',sans-serif] font-semibold text-[15px] text-[#11151c]">{nextAction.desc}</p>
                  </div>
                  <button
                    onClick={() => handleStatusChange(nextAction.value)}
                    disabled={savingStatus}
                    className="flex items-center gap-2 bg-[#b59637] rounded-[100px] px-7 py-3 text-white font-['Lato',sans-serif] font-semibold text-[14px] hover:opacity-90 transition-opacity disabled:opacity-40 whitespace-nowrap"
                  >
                    {savingStatus ? "Menyimpan..." : nextAction.label + " →"}
                  </button>
                </div>
                {statusError && <p className="font-['Lato',sans-serif] text-[13px] text-[#a8312a]">{statusError}</p>}
                <button
                  onClick={() => handleStatusChange("cancelled")}
                  disabled={savingStatus}
                  className="w-fit bg-white text-[#a8312a] border border-[#f5cbc7] hover:bg-[#fdf0ee] rounded-[8px] px-4 py-2 font-['Lato',sans-serif] font-semibold text-[13px] cursor-pointer transition-colors disabled:opacity-50"
                >
                  Batalkan Pesanan
                </button>
              </div>
        ...
        {isShipped && (
          <div className="bg-[#f3eef8] border border-[#d4c5f5] rounded-[16px] px-5 py-5">
            <p className="font-['Lato',sans-serif] text-[11px] font-bold uppercase tracking-wider text-[#6b3fa0] mb-3">Info Pengiriman</p>
            {shipSuccess && (
              <p className="font-['Lato',sans-serif] text-[13px] text-[#2a7a50] bg-[#edf7f2] rounded-[8px] px-4 py-2 mb-3">
            ...
            <div className="flex flex-col gap-2">
              {order.tracking_number && (
                <div>
                  <p className="font-['Lato',sans-serif] text-[11px] text-[#889bbf] mb-0.5">Nomor Resi</p>
                  <p className="font-mono font-bold text-[16px] text-[#11151c]">{order.tracking_number}</p>
                </div>
              )}
              {order.shipping_status && (
                <div>
                  <p className="font-['Lato',sans-serif] text-[11px] text-[#889bbf] mb-0.5">Status Kurir</p>
              ...
              {order.tracking_number && (
                <a href={trackingUrl(order.shipping_courier, order.tracking_number)} target="_blank" rel="noopener noreferrer"
                  className="w-fit mt-1 font-['Lato',sans-serif] text-[13px] font-semibold text-[#6b3fa0] hover:opacity-70">
                  Lacak Paket →
                </a>
              )}
              <div className="border-t border-[#e6ecf7] pt-4 mt-2">
                <p className="font-['Lato',sans-serif] text-[11px] text-[#889bbf] mb-2">Label Pengiriman</p>
                <a
                  href={`/api/admin/orders/${order.order_number}/label?token=${encodeURIComponent(token)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 bg-[#b59637] text-white rounded-[100px] px-6 py-2.5 font-['Lato',sans-serif] font-semibold text-[13px] hover:opacity-90 transition-opacity"
                >
        ...
        {order.voucher_code && (
          <div className="bg-[#fdf6ec] border border-[#f0d89a] rounded-[16px] px-5 py-5">
            <p className="font-['Lato',sans-serif] text-[11px] font-bold uppercase tracking-wider text-[#b59637] mb-3">Info Voucher</p>
            <div className="flex flex-col gap-2">
              <div>
                <p className="font-['Lato',sans-serif] text-[11px] text-[#889bbf] mb-0.5">Kode Voucher</p>
                <p className="font-mono font-bold text-[16px] text-[#11151c]">{order.voucher_code}</p>
              </div>
              {order.voucher_expires_at && (
                <div>
                  <p className="font-['Lato',sans-serif] text-[11px] text-[#889bbf] mb-0.5">Berlaku Sampai</p>
                  <p className="font-['Lato',sans-serif] font-semibold text-[14px] text-[#11151c]">{fmtDate(order.voucher_expires_at)}</p>
                </div>
              )}
              <div className="border-t border-dashed border-[#e8d9a8] pt-3 mt-1">
                {order.voucher_used ? (
                  <p className="font-['Lato',sans-serif] text-[13px] font-semibold text-[#2a7a50]">
                  ...
                  <div className="flex flex-col gap-2 items-start">
                    <button
                      ...
                      className="bg-[#b59637] rounded-[100px] px-6 py-2.5 text-white font-['Lato',sans-serif] font-semibold text-[13px] hover:opacity-90 transition-opacity disabled:opacity-50"
                    >
                      {redeemingVoucher ? "Menyimpan..." : "Tandai Voucher Digunakan"}
                    </button>
                    {voucherError && <p className="font-['Lato',sans-serif] text-[13px] text-[#a8312a]">{voucherError}</p>}
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="bg-white rounded-[16px] border border-[#e6ecf7] px-5 py-5">
          <p className="font-['Lato',sans-serif] text-[11px] font-bold uppercase tracking-wider text-[#889bbf] mb-4">Produk</p>
          <div className="flex flex-col gap-3">
            {order.items?.map((item, i) => (
              <div key={i} className="flex justify-between gap-4">
                <span className="font-['Lato',sans-serif] text-[14px] text-[#11151c] flex items-center gap-2 flex-wrap">
                  {item.name} <span className="text-[#889bbf]">x{item.qty}</span>
                  {isVoucherItem(item, order) && (
                    <span className="px-2 py-0.5 rounded-[100px] bg-[#fdf6ec] border border-[#f0d89a] font-['Lato',sans-serif] font-semibold text-[10px] text-[#b59637] whitespace-nowrap">
                      Voucher
                    </span>
                  )}
                </span>
                <span className="font-['Lato',sans-serif] font-medium text-[14px] text-[#3b4963] whitespace-nowrap">{rupiah(item.price * item.qty)}</span>
              </div>
            ))}
            <div className="border-t border-dashed border-[#dde3f0] pt-3 flex flex-col gap-2">
              ...
              <div className="flex justify-between">
                <span className="font-['Lato',sans-serif] font-extrabold text-[15px] text-[#11151c]">Total</span>
                <span className="font-['Lato',sans-serif] font-extrabold text-[16px] text-[#11151c]">{rupiah(order.total)}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {order.address && (
          <div className="bg-white rounded-[16px] border border-[#e6ecf7] px-5 py-5">
            <p className="font-['Lato',sans-serif] text-[11px] font-bold uppercase tracking-wider text-[#889bbf] mb-3">Alamat</p>
            ...
            {[order.address.detail, order.address.district, order.address.city, order.address.province].filter(Boolean).length > 0 && (
              <p className="font-['Lato',sans-serif] text-[13px] text-[#3b4963] mt-1">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-[20px] max-w-[380px] w-full p-6 flex flex-col gap-4">
            <h3 className="font-['Lato',sans-serif] font-extrabold text-[18px] text-[#11151c]">Konfirmasi Pengiriman</h3>
            ...
            <p className="font-['Lato',sans-serif] text-[12px] text-[#889bbf] bg-[#f9f7f4] rounded-[8px] px-3 py-2">
              Aksi ini akan memproses pengiriman dan tidak bisa dibatalkan.
            </p>
            {shipError && (
              <p className="font-['Lato',sans-serif] text-[13px] text-[#a8312a] bg-[#fdf0ee] rounded-[8px] px-3 py-2">{shipError}</p>
            )}
            <div className="flex gap-3">
              <button onClick={() => setShowModal(false)} disabled={shipping}
                className="flex-1 border border-[#dde3f0] rounded-[100px] py-2.5 font-['Lato',sans-serif] font-semibold text-[14px] text-[#3b4963] hover:border-[#889bbf] disabled:opacity-50">
                Batal
              </button>
              <button onClick={handleShip} disabled={shipping}
                className="flex-1 bg-[#b59637] rounded-[100px] py-2.5 text-white font-['Lato',sans-serif] font-semibold text-[14px] hover:opacity-90 disabled:opacity-50">
